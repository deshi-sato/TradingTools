...
# 2) codes モード（推奨）: 日別IC/TopN + BottomN
if mode == "codes" and all([date_col, id_col, score_col, ret_col]):

    def spearman_ic(g):
        return g[score_col].corr(g[ret_col], method="spearman")
    ...
    # Top/Bottom の日次平均リターンを同時に作成
    frames = []
    for n in (5,10,20):
        s_top = df.groupby(date_col).apply(
            lambda g: g.sort_values(score_col, ascending=False).head(n)[ret_col].mean()
        ).rename(f"top{n}_ret")
        s_bot = df.groupby(date_col).apply(
            lambda g: g.sort_values(score_col, ascending=True).head(n)[ret_col].mean()
        ).rename(f"bot{n}_ret")
        frames += [s_top, s_bot]
    topn_df = pd.concat(frames, axis=1).dropna().reset_index()
    ...
    for n in (5,10,20):
        ...
        summary.append(f"Bottom{n} mean next-ret = {topn_df[f'bot{n}_ret'].mean():.6f}")
...
# 3) base モード: スナップショットのみ（旧互換）
    ...
    if score_col and ret_col:
        ic_val = float(df[score_col].corr(df[ret_col], method="spearman"))
        summary.append(f"IC (Spearman) = {ic_val:.6f}")
        top = {}
        for n in (5,10,20):
