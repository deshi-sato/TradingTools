...
Performs a SELL-side grid search over paper-rule thresholds using features_stream + labels_outcome.
Evaluates precision via day-based GroupKFold, storing the best parameter set after short-side normalization.
"""
...
def _pref_params_key(p: Dict[str, float]) -> tuple:
    return (
        float(p.get("SELL_UPTICK_THR", -1e9)),
        float(p.get("SELL_SCORE_THR", -1e9)),
        float(p.get("VOLUME_SPIKE_THR", -1e9)),
        -float(p.get("SELL_SPREAD_MAX", 1e9)),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Per-trade slippage (in bp, subtracted from realized return).",
    )
...
def ensure_indexes(conn: sqlite3.Connection, ts_column: Optional[str]) -> None:
    statements = [
    ...
    if ts_column:
        conn.execute(
            f"CREATE INDEX IF NOT EXISTS idx_features_symbol_{ts_column} ON features_stream(symbol, {ts_column})"
        )
    else:
        conn.execute(
...
def load_rows(
    ...
    join_condition = "f.symbol = l.symbol AND f.dataset_id = l.dataset_id"
    if ts_column:
        if ts_column not in {"ts_ms", "t_event_ms"}:
            raise SystemExit(f"ERROR: unsupported timestamp column '{ts_column}'")
        join_condition += f" AND f.{ts_column} = l.ts"
    else:
    ...
    df["ret_bp"] = df["ret_bp"].astype("float64") * -1.0
    df["label"] = df["label"].astype("int64")
    df["label"] = (1 - df["label"]).clip(lower=0, upper=1).astype("int64")
    if "f1" in df.columns:
        df["downtick_ratio"] = (1.0 - df["f1"] / 10.0).clip(lower=0.0, upper=1.0)
    else:
        df["downtick_ratio"] = np.nan
    numeric_optional = [
        "f1",
        "downtick_ratio",
        "score",
    ...
    for row in df.itertuples(index=False, name="Row"):
        ...
        rows.append(
            DataRow(
                ...
                label=int(row.label),
                downtick_ratio=_opt(getattr(row, "downtick_ratio", None)),
...
def attach_paths(
    ...
    if ts_column:
        if ts_column not in {"ts_ms", "t_event_ms"}:
            raise SystemExit(f"ERROR: unsupported timestamp column '{ts_column}'")
        ts_select = f"{ts_column} AS ts_ms"
        ts_filter = f"{ts_column} BETWEEN ? AND ?"
        ts_order = ts_column
    else:
        ts_select = "CAST(t_exec * 1000 AS INTEGER) AS ts_ms"
        ts_filter = "CAST(t_exec * 1000 AS INTEGER) BETWEEN ? AND ?"
        ts_order = "ts_ms"
    sql = f"""
        SELECT symbol, {ts_select}, bid1, ask1, spread_ticks, v_rate, f1_delta
...
def load_dataset_cache(
    ...
    if meta.get("version") != CACHE_VERSION:
        return None
    if meta.get("ts_column") != ts_column:
        return None
...
def simulate_trade(
    ...
    for point in path[1:]:
        ...
        if current_mid is None or current_mid <= 0:
            continue
        move_bp = _compute_ret_bp(entry_mid, current_mid)
        if move_bp <= -tp_bp:
            return -move_bp, "tp"
        if move_bp >= sl_bp:
            return -move_bp, "sl"
    ...
    if last_mid is None or last_mid <= 0:
        last_mid = entry_mid
    move_bp = _compute_ret_bp(entry_mid, last_mid)
    return -move_bp, "timeout"
...
def evaluate_on_indices(
    ...
    downtick_thr = float(params["SELL_UPTICK_THR"])
    score_thr = params.get("SELL_SCORE_THR")
    spread_max = float(params["SELL_SPREAD_MAX"])
    cooldown = params["COOLDOWN_SEC"]
    vol_thr = params.get("VOL_SURGE_MIN")
    v_rate_thr = params.get("VOLUME_SPIKE_THR")
    slippage = float(slippage_bp)

    for idx in sorted_idx:
        row = rows[idx]
        down_ratio = row.downtick_ratio
        if down_ratio is None:
            continue
        x = float(down_ratio)
        # 0..10 系が来たら「下優位」に変換して 0..1 正規化
        if x > 1.0:
            # 10 が「上優位（買い優勢）」なら、下優位 = (10 - x)/10
            down_ratio = max(0.0, min(1.0, (10.0 - x) / 10.0))
        else:
            # すでに 0..1 系ならそのまま（念のためクランプ）
            down_ratio = max(0.0, min(1.0, x))
        if down_ratio < downtick_thr:
            continue
...
def evaluate_params(
    ...
    if cv <= 1 or len(days) <= 1:
        ...
        metrics = evaluate_on_indices(rows, indices, params, slippage_bp)
    ...
    for fold_days in folds:
        ...
        if not indices:
            continue
        fold_metrics.append(evaluate_on_indices(rows, indices, params, slippage_bp))
    if not fold_metrics:
...
#    vols = [0.0, 0.5, 1.0] if has_vol else [None]
    vols = [None]
#    volume_spike_thresholds = [1.8, 1.9, 2.0] if has_v_rate else [None]
    volume_spike_thresholds = [1.2, 1.4]
#    stall_vrate_thresholds = [1.1, 1.3, 1.5] if has_v_rate else [None]
    stall_vrate_thresholds = [None]
#    tp_candidates = [12.0, 15.0, 18.0, 21.0]
...
def evaluate_grid(
    ...
    print(f"[grid-SELL] total candidates = {total_candidates}")
    sys.stdout.flush()

    for idx, params in enumerate(params_list, start=1):
        agg = evaluate_params(rows, days, day_groups, params, cv, slippage_bp)
        ...
        if verbose:
            ...
            print(
                "[grid-SELL] %4d/%d (%.1f%%) precision=%.4f trades=%d ev=%.3f params=%s"
                % (
                    idx,
                    total_candidates,
                    progress,
        ...
        if (idx % step == 0) or (idx == total_candidates):
            ...
            if best_eligible:
                ...
                best_str = "best n/a"
            print(
                "[grid-SELL] {done:,}/{total:,} ({pct:4.1f}%)  {cps:,.0f}/s  "
    ...
    print(f"[grid-SELL] evaluated {total_candidates} candidates in {duration:.2f}s")
    print(
        f"[grid-SELL] eligible candidates = {len(eligible_results)}/{total_candidates}"
    )
...
def main() -> None:
    ...
    slippage_bp = float(getattr(args, "SlippageBp", 0.0))
    ...
    if "t_event_ms" in columns:
        ts_column: Optional[str] = "t_event_ms"
    elif "ts_ms" in columns:
        ts_column = "ts_ms"
    else:
        ts_column = None
    ensure_indexes(conn, ts_column)
    ...
    if "f1" not in columns:
        raise SystemExit(
            "ERROR: features_stream missing f1 (uptick ratio) column (needed to derive downtick bias)."
        )
    ...
    if cache_hit:
        ...
        if rows and not getattr(rows[0], "path", None):
            attach_paths(
                conn, dataset_id, rows, ts_column, MAX_HOLD_DEFAULT, PATH_STEP_MS
            )
        print(f"[grid-SELL] dataset cache hit (rows={len(rows)})")
    else:
        rows = load_rows(
            conn,
            dataset_id,
        ...
        attach_paths(conn, dataset_id, rows, ts_column, MAX_HOLD_DEFAULT, PATH_STEP_MS)
        ...
        print(f"[grid-SELL] dataset cache stored (rows={len(rows)})")
    conn.close()
    ...
    if v_rate_summary:
        print(
            "[grid-SELL] v_rate stats: p50=%.2f p95=%.2f p99=%.2f max=%.2f (n=%d)"
            % (
                v_rate_summary["p50"],
                v_rate_summary["p95"],
                v_rate_summary["p99"],
    ...
    if symbols_filter:
        print(f"[grid-SELL] symbol filter: {', '.join(symbols_filter)}")
    print(
        f"[grid-SELL] applying filters: MinTrades>={min_trades:,}, EVFloor>={ev_floor:.3f}"
    )
    if slippage_bp:
        print(f"[grid-SELL] EV slippage adjustment per trade: {slippage_bp:.3f}bp")
    ...
    for idx, item in enumerate(top_results, start=1):
        ...
        print(
            "[grid-top-SELL] #%d precision=%.4f trades=%d EV=%.3f eligible=%s params=%s"
            % (
                idx,
                item["precision"],
                item["signals"],
    ...
    if eligible_results:
        ...
        print(
            "[grid-SELL] selecting best among all candidates for reference: "
    ...
    if args.Out:
        ...
        if out_arg.suffix.lower() == ".json":
            ...
            out_path = out_arg / f"best_thresholds_sell_{dataset_id}.json"
    else:
        out_path = Path(f"exports/best_thresholds_sell_{dataset_id}.json")

    write_json(out_path, best_payload)

    report_path = Path(f"exports/grid_report_sell_{dataset_id}.csv")
    ...
    if eligible_results and len(eligible_results) != len(all_results):
        filtered_report_path = Path(f"exports/grid_report_eligible_sell_{dataset_id}.csv")
        write_csv_report(filtered_report_path, eligible_results)

    candidates_path = out_path.parent / f"grid_candidates_sell_{dataset_id}.csv"
    write_candidates_csv(candidates_path, filtered_results)
    all_candidates_path = out_path.parent / f"grid_candidates_all_sell_{dataset_id}.csv"
    write_candidates_csv(all_candidates_path, all_results)

    print(f"[grid-SELL] best saved to {out_path}")
    print(f"[grid-SELL] report saved to {report_path}")
    if filtered_report_path:
        print(f"[grid-SELL] eligible report saved to {filtered_report_path}")
    print(f"[grid-SELL] candidates saved to {candidates_path}")
    print(f"[grid-SELL] all candidates saved to {all_candidates_path}")
