# 1) 念のためバックアップ
Copy-Item .\scripts\naut_runner.py .\scripts\naut_runner.broken.backup.py -Force

# 2) 修正版を書き込み（ここから終わりの @' まで全部コピペ）
@'
            ...
            for s in symbols:
                ...
                for f in rows:
                    ...
                    if buyish:
                        ...
                        if not poll.consistent(s, +1, cfg["WINDOW"]):
                            skips["streak"] += 1
                            continue
...
if __name__ == "__main__":
    main()
'@ | Set-Content -Encoding UTF8 .\scripts\naut_runner.py
