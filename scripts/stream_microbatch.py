...
class PushTickReceiver(threading.Thread):
    ...
    def _enqueue(self, event: Tuple[str, float, int, str]) -> None:
        ...
        payload = (symbol, price, int(max(size, 0)), ts_iso)
        try:
...
def main() -> None:
    ...
    logging.info("stream_microbatch start (mode=%s, host=%s, port=%s)", mode, host, port)
    ...
    print(
        f"[BOOT] mode={mode} source={'CLI' if symbols_cli else 'CONFIG'} "
        f"host={host}:{port} count={len(symbols_final)} symbols={symbols_final} "
        f"db_path={db_path}"
    )
    ...
    if mode == "online":
        ...
        tick_thread = MockTickReceiver(symbols_final, q, stop_event, interval_ms=mock_interval)
