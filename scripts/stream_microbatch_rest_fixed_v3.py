...
"""
stream_microbatch_rest_fixed_v3.py

- CurrentPrice (PUSH via kabu WS) -> tick_batch
- Board (REST /board/{symbol}@1) -> orderbook_snapshot
- Derived features -> features_stream

Robustness:
- Accept multiple JSON key variants from /board
- Fallback to latest DB board snapshot when REST board is empty
- BOM-safe JSON loader for config files
"""
...
class PushTickReceiver(threading.Thread):
    ...
    def _on_msg(self, payload: Any):
        ...
        lo, hi = lo_hi if isinstance(lo_hi, (list, tuple)) else (None, None)
    ...
    try:
        while True:
            ...
            for s in symbols:
                ...
                # poll board once for this symbol during the window
