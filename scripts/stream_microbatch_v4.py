...
"""
stream_microbatch.py (keepalive + backpressure 対応)
- kabu WebSocket PUSH からティックを受信し tick_batch / features_stream に書き込み
- orderbook_snapshot は ws_board_receiver 側が書く想定（未存在でも処理継続）
"""
...
class PushTickReceiver(threading.Thread):
    def __init__(
        ...
        *,
        ...
        price_guard,
        connect_timeout=6.0,
        recv_timeout=5.0,      # 監視用
        keepalive_sec=20,      # PING 周期
        idle_reconnect_sec=30  # 無通信で再接続
    ):
                ...
                while not self.stop_event.is_set():
                    ...
                    if now - last_ping >= self.keepalive_sec:
                        try:
                            ...
                            raise RuntimeError("ping failed -> reconnect")
                        last_ping = now
                    ...
                    try:
                        ...
                        # 無通信監視
                        if time.time() - last_rx > self.idle_reconnect_sec:
    ...
    def _on_msg(self, payload: str):
        try:
            obj = json.loads(payload)
        except Exception:
            return

        # Symbol / IssueCode 両対応
        ...
        # backpressure: 満杯なら最古を捨てて投入
        try:
            self.q.put((sym, float(price), int(qty), ts), timeout=0.01)
        except queue.Full:
            try:
                self.q.get_nowait()
            except queue.Empty:
                pass
            try:
                self.q.put_nowait((sym, float(price), int(qty), ts))
            except queue.Full:
                logger.warning("queue full; drop newest")
...
def main() -> None:
    ...
    symbols = [str(x).split("@")[0] for x in cfg.get("symbols", [])]  # "8136@1" → "8136"
    ...
    qmax          = int(cfg.get("tick_queue_max", 20000))
    ...
    rx = PushTickReceiver(
        symbols, q, stop_event,
        host=host, port=port, token=token, price_guard=price_guard,
        connect_timeout=6.0, recv_timeout=5.0, keepalive_sec=20, idle_reconnect_sec=30
    )
    rx.start()
    ...
    try:
        while True:
            ...
            while time.monotonic() < next_cut:
                ...
                # キュー水位を5秒毎にログ
                if time.monotonic() - last_q_log >= 5.0:
                    last_q_log = time.monotonic()
                    logger.info("qsize=%d/%d", q.qsize(), qmax)
